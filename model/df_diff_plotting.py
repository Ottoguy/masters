...
def create_plot(id_value, data):
    ...
    if path.exists(results_dir + "/" + plot_name):
        ...
        for row in data:
            ...
            current.append(float(row[16]))
            voltage.append(float(row[15]))
            status.append(row[8])
        ...
        ax1.plot(timestamps, current, label="Diff", linestyle='--')
        ...
        ax2.plot(timestamps, voltage, label="Diff", linestyle='--')
