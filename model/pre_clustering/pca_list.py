...
# Group by 'ID' and create lists for each feature
grouped_data = data.groupby('ID').agg(lambda x: x.tolist()).reset_index()

# Separate numerical and categorical columns after grouping by 'ID'
...
categorical_cols = ['ChargingStatus', "ChargingPoint", "ID"]  # Assuming there's an 'ID' column

# Drop non-numeric columns for PCA
numerical_data = grouped_data[numerical_cols]
...
# Merge with grouped data on 'ID'
principal_df = pd.merge(grouped_data, principal_df, on='ID')
...
for i, (pc1, pc2) in enumerate(combinations):
    #print(f'Loop {i}...')
    row = i // 2  # Correct the number of columns in the subplot
    col = i % 2
